    ...
    const renderField = (label: string, name: keyof typeof customer, type: string = 'text') => (
        <div>
            ...
            <input
                ...
                className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm text-black focus:outline-none focus:ring-brand-dark-pink focus:border-brand-dark-pink"
                ...
                <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }} className="p-6">
                    ...
                    <fieldset className="border p-4 rounded-md mb-4">
                        ...
                        <div className="relative mb-2">
                            <label className="block text-sm font-medium text-gray-700">Search Item (by S/N or Name)</label>
                            <input 
                                type="text"
                                placeholder="e.g., ROXXY-001 or Star Sneakers"
                                value={searchTerm}
                            ...
                            {searchResults.length > 0 && searchTerm && (
                                <ul className="absolute z-10 w-full bg-white border border-gray-300 rounded-md mt-1 max-h-40 overflow-y-auto">
                                    {searchResults.map(p => (
                                        <li key={p.id} onClick={() => handleSelectProduct(p)} className="px-3 py-2 cursor-pointer hover:bg-gray-100">{p.id} - {p.name}</li>
                                    ))}
                                </ul>
                            )}
                        </div>
                        ...
                        {selectedProduct && (
                             <div className="bg-gray-50 p-3 rounded-md grid grid-cols-1 md:grid-cols-4 gap-3 items-end">
                                ...
                                <div>
                                    ...
                                    <select value={selectedColor} onChange={e => setSelectedColor(e.target.value)} className="w-full border p-2 rounded bg-white text-black mt-1">
                                        {selectedProduct.colors.map(c => <option key={c} value={c}>{c}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-sm">Size</label>
                                     <select value={selectedSize} onChange={e => setSelectedSize(e.target.value)} className="w-full border p-2 rounded bg-white text-black mt-1">
                                        {selectedProduct.sizes.map(s => <option key={s} value={s}>{s}</option>)}
                                    </select>
                                </div>
                                <div>
                                    <label className="text-sm">Quantity</label>
                                    <input type="number" min="1" value={quantity} onChange={e => setQuantity(parseInt(e.target.value))} className="w-full border p-2 rounded bg-white text-black mt-1" />
                                </div>
                    ...
                    <div className="flex justify-end space-x-4 mt-6">
                        ...
                        <button type="submit" className="bg-brand-text text-white py-2 px-4 rounded-lg">Add Order</button>
                    </div>
                </form>
            </div>
        </div>
...
const PaymentModal: React.FC<{ order: Order; onClose: () => void; onConfirm: (remarks: string) => void; }> = ({ order, onClose, onConfirm }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-[60] flex justify-center items-center">
            <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-md">
                ...
                <p>You are marking this order as delivered. Please confirm payment has been received.</p>
                ...
                <div>
                    ...
                    <input
                        ...
                        className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-dark-pink focus:border-brand-dark-pink"
                        placeholder="e.g., Paid via Cash"
                    />
                </div>
                <div className="flex justify-end space-x-4 mt-6">
                    <button type="button" onClick={onClose} className="bg-gray-300 py-2 px-4 rounded-lg">Cancel</button>
                    <button type="button" onClick={handleSubmit} className="bg-green-600 text-white py-2 px-4 rounded-lg">Confirm & Add Payment</button>
                </div>
            </div>
        </div>
    );
...
const AdminOrderHistory: React.FC = () => {
    const { orders, updateOrderStatus, addShopOrder, addPaymentForOrder, currentUser } = useStore();
    ...
    const filteredAndSortedOrders = useMemo(() => {
        ...
        // Apply filter
        if (filterType !== 'All') {
            sortableItems = sortableItems.filter(order => order.type === filterType);
        }
        ...
        sortableItems.sort((a, b) => {
            if (sortConfig.key === 'status') {
                const orderA = statusOrder[a.status] + a.createdAt.getTime() / 1e14;
                const orderB = statusOrder[b.status] + b.createdAt.getTime() / 1e14;
    ...
    const handleConfirmPayment = (remarks: string) => {
        if (viewingOrder) {
            addPaymentForOrder(viewingOrder, remarks);
    ...
    return (
        <div className="p-6">
                 ...
                 <div className="flex items-center gap-4">
                    <div className="relative">
                        <select
                            ...
                            className="appearance-none bg-white border border-gray-300 rounded-md py-2 pl-3 pr-8 text-sm leading-tight focus:outline-none focus:border-brand-dark-pink"
                        >
                ...
                <table className="w-full text-sm text-left text-gray-500">
                    <thead className="text-xs text-gray-700 uppercase bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" className="px-6 py-3 cursor-pointer" onClick={() => handleSort('date')}>Date</th>
                    ...
                    <tbody>
                        {filteredAndSortedOrders.map((order, index) => (
                            <tr key={order.id} className="bg-white border-b">
                                ...
                                <td className="px-6 py-4">
                                    <button onClick={() => setViewingOrder(order)} className="text-blue-600 hover:text-blue-800 font-medium">View Details</button>
                                </td>
                            </tr>
                        ))}
                         {filteredAndSortedOrders.length === 0 && (
                            <tr><td colSpan={8} className="text-center py-8 text-gray-500">No orders match the current filter.</td></tr>
                         )}
                    </tbody>
                </table>
            </div>
                    ...
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                {viewingOrder.customer.location && 
                                <p><a href={`https://www.google.com/maps?q=${viewingOrder.customer.location.lat},${viewingOrder.customer.location.lng}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">View on Google Maps</a></p>
                                }
                            </div>
                            <div>
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    <select value={viewingOrder.status} disabled={!canChangeStatus} onChange={(e) => handleStatusChange(viewingOrder, e.target.value as OrderStatus)} className="p-1 border rounded disabled:bg-gray-100 disabled:cursor-not-allowed">
                                ...
                                {viewingOrder.paymentScreenshot && (
                                    <div>
                                        ...
                                        <a href={viewingOrder.paymentScreenshot} target="_blank" rel="noopener noreferrer">
                                            <img src={viewingOrder.paymentScreenshot} alt="Payment" className="max-w-xs mt-1 rounded border"/>
                                        </a>
                                    </div>
                                )}
                            </div>
            ...
            {isPaymentModalOpen && viewingOrder && <PaymentModal order={viewingOrder} onClose={() => setIsPaymentModalOpen(false)} onConfirm={handleConfirmPayment} />}
        </div>
    );
};
